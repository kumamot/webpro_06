...
このプログラムを起動するには、以下の手順を実行する必要がある．

1. **リポジトリのクローン**
   - GitHubからプロジェクトをローカルにクローンする．
     ```bash
     git clone https://github.com/kumamot/webpro_06.git
     ```
   - または、GitHubリポジトリページからZIPをダウンロードして解凍する．
...
3. **依存パッケージのインストール**
   - 必要なパッケージをインストールする．（Node.jsがインストールされていることを前提とする）．
     ```bash
     npm install
     ```
...
4. **サーバーを起動**
   - 以下のコマンドでサーバーを起動する．
     ...
     node app5.js
     ```
   - 起動後、ブラウザで以下のURLにアクセスして動作を確認できる．
     ```
     http://localhost:8080
     ```
...
プロジェクトの編集や変更をGitで管理するには、以下の手順を実行する必要がある．
...
2. **変更をステージング**
   - 変更したファイルをステージングする．
     ...
     git add .
     ```
   - または、特定のファイルだけをステージングする場合は以下のようにする．
     ```bash
     git add <ファイル名>
     ```
...
5. **GitHubでの確認**
   - ブラウザでGitHubリポジトリを開き，変更が反映されていることを確認する．
